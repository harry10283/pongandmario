...
function push:initValues()
  local pixelScale = love.window and love.window.getDPIScale() or 1

  self._SCALE = {
      x = self._RWIDTH / self._WWIDTH * pixelScale,
      y = self._RHEIGHT / self._WHEIGHT * pixelScale
  }
  ...
  self._GWIDTH = self._RWIDTH * pixelScale - self._OFFSET.x * 2
  self._GHEIGHT = self._RHEIGHT * pixelScale - self._OFFSET.y * 2
end
...
function push:finish(shader)
  ...
  if self._canvas then
    ...
    -- Draw render
    love.graphics.translate(self._OFFSET.x, self._OFFSET.y)
    love.graphics.setShader(shader or _render.shader)
    love.graphics.draw(_render.canvas, 0, 0, 0, self._SCALE.x, self._SCALE.y)
...
function push:switchFullscreen(winw, winh)
  ...
  elseif not self._WINWIDTH or not self._WINHEIGHT then
    self._WINWIDTH, self._WINHEIGHT = windowWidth * 0.5, windowHeight * 0.5
  end
